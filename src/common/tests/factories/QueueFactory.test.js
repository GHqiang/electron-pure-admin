...
import { offerQueueFactory, ticketQueueFactory } from "../../factories/QueueFactory.js";
...
describe("TicketQueueFactory", () => {
  test("应该能够创建出票队列实例", () => {
    const queue = ticketQueueFactory.createTicketQueue("lma");
    
    expect(queue).toBeDefined();
    expect(queue.appFlag).toBe("lma");
  });

  test("应该根据影院类型创建不同的出票队列", () => {
    // 测试不同类型的影院
    const commonQueue = ticketQueueFactory.createTicketQueue("test_cinema");
    expect(commonQueue).toBeDefined();
  });
});
