...
import umeApi from "@/api/haha-api";
...
class OrderAutoTicketQueue {
  ...
  async releaseSeat(unlockSeatInfo, flag) {
    const { conPrefix } = this;
    const { cinemaCode, cinemaLinkId, scheduleId, scheduleKey, session_id } =
      unlockSeatInfo;
    try {
    ...
    try {
      ...
      if (this.currentParamsInx === 0) {
        ...
        if (!cinemaList?.length) {
          console.error(conPrefix + "获取目标城市影院列表失败");
        ...
        if (!seatList?.length) {
          console.error(conPrefix + "获取座位布局为空");
          this.setErrInfo("获取座位布局为空");
          this.logList.push({
            opera_time: getCurrentFormattedDateTime(),
            des: `获取座位布局为空`
          });
      ...
      let lockRes;
      try {
        lockRes = await this.lockSeat(params); // 锁定座位
        // this.logList.push({
        //   opera_time: getCurrentFormattedDateTime(),
        //   des: `首次锁定座位成功`
        // });
  ...
  async getCityCinemaList() {
    ...
    try {
      ...
      this.setErrInfo("获取城市影院列表异常", error);
    }
  }
  ...
  async getMoviePlayInfo(data) {
    ...
    try {
      ...
      const res = await umeApi.getMoviePlayInfo(params);
      ...
      console.error(conPrefix + "获取影院放映列表异常", error);
      this.setErrInfo("获取影院放映列表异常", error);
    }
  }
  ...
  async getMoviePlayDate(data) {
    ...
    try {
      ...
      const res = await umeApi.getMoviePlayDate(params);
      console.log(conPrefix + "获取电影放映日期返回", res);
      return res.data || [];
    } catch (error) {
      console.error(conPrefix + "获取电影放映日期异常", error);
      this.setErrInfo("获取电影放映日期异常", error);
    }
  }
  // 获取电影放映场次
  async getMoviePlayTime(data) {
    ...
    try {
      ...
      const res = await umeApi.getMoviePlayTime(params);
  ...
  async getSeatLayout(data, flag = 0) {
    const { conPrefix } = this;
    try {
      let { cinemaCode, cinemaLinkId, scheduleId, scheduleKey, session_id } =
        data || {};
      let params = {
        cinemaCode,
        cinemaLinkId,
      ...
      const res = await umeApi.getMoviePlaySeat(params);
  ...
  async lockSeat(data, inx = 1) {
    ...
    try {
      ...
      const res = await umeApi.lockSeat(params);
  ...
  async getOptimalCardQuanCompose(data) {
    try {
      ...
      console.error("获取最优卡券组合异常", error);
    }
  }
  ...
  async priceCalculation(data) {
    ...
    try {
      ...
      const res = await umeApi.priceCalculation(params);
    ...
    try {
      ...
      if (card_id) {
        ...
        try {
          ...
          const res = await umeApi.createOrder(params);
  ...
  async buyTicket(data) {
    ...
    try {
      ...
      const res = await umeApi.buyTicket(params);
  ...
  async payOrder(data) {
    ...
    try {
      ...
      const res = await umeApi.payOrder(params);
      ...
      try {
        const res = await umeApi.getOrderList(params);
  ...
  // 根据订单name获取目标影院
  getTargetCinema(cinema_name, list) {
    const { conPrefix, appFlag } = this;
    try {
      // 1、先全字匹配，匹配到就直接返回
  ...
  async getQuanList(data) {
    ...
    try {
      ...
      const res = await umeApi.getQuanList(params);
  ...
  async bandQuan({ coupon_num, session_id }) {
    ...
    try {
      await this.delay(1);
      const res = await umeApi.bandQuan(params);
      // console.log("res", res);
  ...
  async getCardList(data) {
    ...
    try {
      ...
      const res = await umeApi.getCardList(params);
