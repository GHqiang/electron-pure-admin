...
import { LAINA_SPECIAL_CINEMA_LIST } from "@/common/constant";
...
const appTicketRuleList = computed(() =>
  appRuleListStore.items.filter(item => item.appName === "laina")
);
    ...
    while (this.isRunning) {
      ...
      while (this.queue.length > 0 && this.isRunning) {
        ...
        if (order) {
          ...
          // 从缓存里面删除记录
          deleteOrder(order.order_number, "laina");
          // 添加订单处理记录
  ...
  async fetchOrders(fetchDelay) {
    try {
      ...
      let sfcStayOfferlist = getOrginValue(stayTicketList.items).filter(item =>
        item.cinema_name.includes("莱纳龙域影城")
      );
  ...
  async addOrderHandleRecored(order, res) {
    try {
      ...
      const serOrderInfo = {
        ...
        app_name: res?.offerRule?.app_name || "laina",
        order_id: order.id,
...
async function createOrder(data) {
  try {
    ...
    let params = {
      ...
      phone: lainaUserMobile.value || "", // 用户手机号
      additional_goods_info: "", // 附加商品信息
      companion_info: "", // 携伴信息
      goods_info: "", // 商品信息
      option_goods_info: "", // 可选的额外商品信息
...
async function buyTicket(data) {
  try {
    ...
    let params = {
      city_id,
      cinema_id,
      open_id: "oPKih4oNM3oGJGcEItWiN5lJ93oA", // 微信openId
      order_num, // 订单号
      pay_money, // 支付金额
