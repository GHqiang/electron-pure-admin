...
import { NINGBO_SPECIAL_CINEMA_LIST } from "@/common/constant";
import {
...
const { ningboUserMobile } = storeToRefs(userInfoAndTokens);
...
const appTicketRuleList = computed(() =>
  appRuleListStore.items.filter(item => item.appName === "ningbo")
);

const cityList = ref([]); // 城市列表
// 特殊的名字匹配集合
let specialCinemaNameMatchList = NINGBO_SPECIAL_CINEMA_LIST;

let conPrefix = "【宁波影都自动出票】——"; // console打印前缀
    ...
    while (this.isRunning) {
      ...
      while (this.queue.length > 0 && this.isRunning) {
        ...
        if (order) {
          ...
          // 从缓存里面删除记录
          deleteOrder(order.order_number, "ningbo");
          // 添加订单处理记录
  ...
  async addOrderHandleRecored(order, res) {
    try {
      ...
      const serOrderInfo = {
        ...
        app_name: res?.offerRule?.app_name || "ningbo",
        order_id: order.id,
...
async function createOrder(data) {
  try {
    ...
    let params = {
      ...
      phone: ningboUserMobile.value || "", // 用户手机号
      additional_goods_info: "", // 附加商品信息
      companion_info: "", // 携伴信息
      goods_info: "", // 商品信息
      option_goods_info: "", // 可选的额外商品信息
...
async function buyTicket(data) {
  try {
    ...
    let params = {
      city_id,
      cinema_id,
      open_id: "o1TM95HQabEWcsC6u2S0XPd4ge5w", // 微信openId
      order_num, // 订单号
      pay_money, // 支付金额
