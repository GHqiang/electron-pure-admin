...
export default class UmeBuyTicket extends BaseBuyTicket {
   ...
   * 验证待出票订单JSON
   *
   * 用于验证待出票订单的格式和核心方法的可执行性，不实际进行出票
   *
   ...
   * @param {string} orderJson.lockseat - 座位信息（必填）
   ...
   *     - cardInfo: boolean，卡信息获取（如果适用）
   *     - quanInfo: boolean，券信息获取（如果适用）
   */
  async validateTicketOrder() {
    ...
    try {
      ...
      const cinemaInfoRes = await this.cinemaManage.getBuyPrevCinemaInfo({
        ...
        city_name: orderJson.city_name
      });
      if (cinemaInfoRes?.error || !cinemaInfoRes?.cinemaCode) {
      ...
      let targetShow = showList?.find(
        item => +new Date(item.showDateTime) == +new Date(orderJson.show_time)
      );
      if (!targetShow) {
        result.errMsg = "匹配场次失败";
      ...
      if (seatDataRes?.error || !seatDataRes?.seatList?.length) {
      ...
      if (this.offerRule.offer_type !== "1") {
        // 用卡
        const usableCards = await this.cardQuanManage.getUsableCardList(
          cinemaInfoRes.cinemaCode,
          orderJson.ticket_num
        );
        if (!usableCards?.length) {
          result.errMsg = "无可用会员卡";
          result.steps.cardInfo = false;
          return result;
        }
        result.steps.cardInfo = true;
      } else {
        // 用券
        const sortMobileList =
          await this.cardQuanManage.getSortPhoneByQuanTypeList(
        ...
        if (!sortMobileList?.length) {
          result.errMsg = "无可用优惠券";
          result.steps.quanInfo = false;
          return result;
        }
        result.steps.quanInfo = true;
      }
