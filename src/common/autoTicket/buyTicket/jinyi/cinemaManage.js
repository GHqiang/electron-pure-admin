...
export default class CinemaManage {
  ...
  async getBuyPrevCinemaInfo({ flag, cardQuanManage }) {
    try {
      ...
      cinemaInfo.cinema_id = targetCinema.cinema_id;
      cinemaInfo.cinema_name = targetCinema.cinema_name;
      cinemaInfo.cinema_code = targetCinema.cinema_code;
      ...
      // 7、获取影片放映场次
      cinemaInfo.show_date = show_time.split(" ")[0].replaceAll("-", "");
      ...
      cinemaInfo.schedule_id = targetShow.schedule_id;
      cinemaInfo.hall_id = targetShow.hall_id;
    ...
    try {
      ...
      cityCinemaList = cityCinemaList.map(item => ({
        ...
        cinemaList: item.cinemas.map(itemA => ({
          ...itemA,
          cinema_id: itemA.cinema_id,
          cinema_name: itemA.cinema_name,
          cinema_code: itemA.cinema_code
        }))
      }));
      if (!cityCinemaList?.length) {
  ...
  getTargetCinemaInfo(cinema_code, cinemaList) {
    let targetCinema = cinemaList.find(
      item => cinema_code && item.cinema_code == cinema_code
    );
    if (!targetCinema) {
  ...
  getTargetMovie(movieData, film_name) {
    ...
    if (!movieInfo) {
      // 模糊匹配
      this.logger.warn("全字匹配目标影片信息失败", { movieData, film_name });
    }
    return movieInfo;
  }
  ...
  // 获取影院放映信息
  async getMoviePlayInfo({ cinema_id }) {
    try {
      let params = {
        cinema_id
      };
  ...
  async getMoviePlayTime(cinemaInfo) {
    const { cinema_id, film_id, show_date } = cinemaInfo;
    try {
      let params = {
        cinema_id,
        movie_id: film_id
      ...
      let showList = res?.data?.[show_date]?.schedules || [];
      if (!showList?.length) {
  ...
  async getTargetShow(cinemaInfo, retryNextDay = true) {
    try {
      ...
      if (retryNextDay) {
        ...
        let show_date = cinemaInfo.show_date;
        // 将'20251226'格式化为'2025-12-26'，再取前一天，最后转回'YYYYMMDD'保持与API key格式一致
        show_date =
          show_date.slice(0, 4) +
          "-" +
          show_date.slice(4, 6) +
          "-" +
          show_date.slice(6, 8);
        const prevDay = getPreviousDay(show_date);
        const prevDayCompact = prevDay.replaceAll("-", "");
        return this.getTargetShow(
          {
            ...cinemaInfo,
            show_date: prevDayCompact
          },
          false
        );
      }
  ...
  _findTargetShow(showList) {
    ...
    const [showDate, showTime] = show_time.split(" ");
    let targetShowList = showList.filter(
      item =>
        item.show_date === showDate.replaceAll("-", "") &&
        item.show_time == showTime.slice(0, 5)
    );
    ...
    targetShowList = targetShowList.map(item => ({
      ...item,
      ...findMostRepeatedChars(item.hall_name, hall_name, "hall_name")
    }));
   ...
   * jinyi 字段为下划线：cinema_id / film_id；show_date 格式 YYYYMMDD（去横杠）
  ...
  buildCinemaInfoFromCache(cachedIds) {
    ...
    return {
      ...
      cinema_code: this.order.cinema_code,
      cinema_name: cinema_name,
      film_id: cachedIds.film_id || null,
      show_date: show_time.split(" ")[0].replaceAll("-", ""),
      _fromCache: true
    };
  }
