...
import { JINJI_SPECIAL_CINEMA_LIST } from "@/common/constant";
...
const { jinjiUserMobile } = storeToRefs(userInfoAndTokens);
...
const appTicketRuleList = computed(() =>
  appRuleListStore.items.filter(item => item.appName === "jinji")
);
    ...
    while (this.isRunning) {
      ...
      while (this.queue.length > 0 && this.isRunning) {
        ...
        if (order) {
          ...
          // 从缓存里面删除记录
          deleteOrder(order.order_number, "jinji");
          // 添加订单处理记录
  ...
  async addOrderHandleRecored(order, res) {
    try {
      ...
      const serOrderInfo = {
        ...
        app_name: res?.offerRule?.shadowLineName || "jinji",
        order_id: order.id,
...
async function createOrder(data) {
  try {
    ...
    let params = {
      ...
      phone: jinjiUserMobile.value || "", // 用户手机号
      additional_goods_info: "", // 附加商品信息
      companion_info: "", // 携伴信息
      goods_info: "", // 商品信息
      option_goods_info: "", // 可选的额外商品信息
