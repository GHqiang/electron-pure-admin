...
export default class MayiOfferQueue extends BaseOfferQueue {
  ...
  /**
   * 覆盖基类的截止时间检查决策：
   * 蚂蚁接口已返回 lastBaojiaTime（真实最后报价时间），offer_end_time 为真实截止时间，
   * 各检查点应恢复校验，返回 false（不跳过）。
      ...
      const processedList = stayList
        .map(item => {
          const {
            ...
            lastBaojiaTime // 最后报价时间（接口返回的真实报价截止时间）
          } = item;
          return {
            plat_name: "mayi",
            id: tradeno,
            ...
            // 蚂蚁订单报价截止时间：使用接口返回的 lastBaojiaTime（真实最后报价时间）
            offer_end_time: +new Date(lastBaojiaTime)
          };
        })
        .filter(item => {
