// 票圣平台订单获取
// 继承BaseOrderFetcher，实现票圣平台特定的订单获取逻辑

import BaseOrderFetcher from "../../core/BaseOrderFetcher.js";
import PiaoShengAdapter from "../adapters/PiaoShengAdapter.js";
 ...
 * 票圣平台订单获取
 */
export default class PiaoShengOrderFetcher extends BaseOrderFetcher {
  /**
   * 构造函数
  ...
  constructor(isTestOrder = false) {
    ...
    const adapter = new PiaoShengAdapter(logger, isTestOrder);
    super(adapter, "piaosheng", isTestOrder);
  }
  ...
  async fetchOrders() {
    try {
      const rawStayList = await this.piaoshengOrderFetch();
      ...
      if (diagnoseLogs.length) {
        logUpload(
          { plat_name: "piaosheng", app_name: "", order_number: "", type: 2 },
          diagnoseLogs
        );
      }
      ...
      if (newOrders?.length && !this.isTestOrder) {
        ...
        finalOrders.forEach(item => {
          ...
          this.logOrderEvent(item, "info", "票圣新的待出票订单", {
            newOrder: item,
  ...
  async piaoshengOrderFetch() {
    try {
      ...
      console.error("获取票圣待出票列表异常", error);
      this.logger.errorSave("获取票圣待出票列表异常", { error });
      return [];
    }
  }
  ...
  async getOfferList() {
    try {
      ...
      console.error("获取票圣历史报价记录异常", error);
      this.logger.errorSave("获取票圣历史报价记录异常", { error });
      return [];
    }
  }
  ...
  async getTicketList() {
    try {
      ...
      console.error("获取票圣历史出票记录异常", error);
      this.logger.errorSave("获取票圣历史出票记录异常", { error });
      return [];
    }
  }
}
