...
class OrderAutoOfferQueue {
  constructor() {
    ...
    this.conPrefix = "【影划算自动报价】——"; // console打印前缀
    this.logList = []; // 队列运行日志
  }
      ...
      this.logList.push({
        ...
        info: {
          newOrders: stayList.filter(item =>
            newOrders.some(itemA => itemA.order_number === item.inv_id)
          )
        }
      });
      return newOrders;
  ...
  async submitOffer({ inv_id, quote_price }) {
    ...
    try {
      ...
      const res = await yinghuasuanApi.submitOffer(params);
  ...
  stop() {
    ...
    logUpload({ plat_name: "yinghuasuan", type: 1 }, this.logList);
    // 打印处理结果
    const { handleSuccessOrderList, handleFailOrderList } = this;
    console.warn(
      conPrefix +
