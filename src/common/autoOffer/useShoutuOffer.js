...
import shoutuApi from "@/api/shoutu-api"; // 守兔平台api
...
class OrderAutoOfferQueue {
  constructor() {
    ...
    this.conPrefix = "【守兔自动报价】——"; // console打印前缀
    this.logList = []; // 队列运行日志
  ...
  // 提交报价
  async submitOffer({ orderUUID, userUUID, price }) {
    const { conPrefix } = this;
    let params = { orderUUID, userUUID, price };
    try {
      ...
      const res = await shoutuApi.submitOffer(params);
  ...
  async singleOffer({ order, offerList }) {
    ...
    try {
      ...
      const userUUID = window.localStorage.getItem("shoutuPlatUserUUID") || "";
  ...
  // 获取待报价订单列表
  async getStayOfferList(pageNo = 1, stayList = []) {
    const { conPrefix } = this;
    try {
      const res = await shoutuApi.queryStayOfferList({
        pageNo,
        pageSize: 50 // 最多50条
      });
      let list = res.data.list || [];
      let totalPage = res.data.totalPage || 1; // 总数
      stayList = [...stayList, ...list];
      console.log(pageNo, totalPage);
      console.log(conPrefix + "获取待报价列表返回", stayList);
      if (pageNo >= totalPage) {
        return stayList;
      } else {
        return await this.getStayOfferList(++pageNo, stayList);
      }
    } catch (error) {
