  ...
  dataTableStore.items
    .filter(item =>
      item.platOfferList?.length
        ? item.platOfferList.map(item => item.platName).includes("haha")
        : item.orderForm.split(",").includes("haha")
    )
    .map(itemA => {
      ...
      return {
        ...
        offerAmount:
          itemA.offerType === "1"
            ? itemA.platOfferList?.find(item => item.platName === "haha")
                ?.value || offerAmount
            : itemA.offerType === "3"
              ? offerAmount
              : "",
...
class OrderAutoOfferQueue {
  ...
  stop() {
    ...
    logUpload({ plat_name: "haha", type: 1 }, this.logList);
    // 打印处理结果
    const { handleSuccessOrderList, handleFailOrderList } = this;
    console.warn(
      conPrefix +
