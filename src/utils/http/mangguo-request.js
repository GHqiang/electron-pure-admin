...
instance.interceptors.request.use(
  config => {
    if (config.url.indexOf("/v2/") !== -1) {
      ...
      const token = tokens.mangguoToken || "";
      if (token) {
      ...
      config.url = IS_DEV
        ? config.url
        : "https://supplier.mgmovie.net" + config.url;
    }
...
instance.interceptors.response.use(
  response => {
    ...
    let isErrorByLieRen =
      response.config.url.indexOf("/v2/") !== -1 && data.code !== 1;
    if (
      isErrorByLieRen &&
      !whitelistSp.some(item => response.config.url.includes(item))
    ) {
      ElMessage.error(data.message || "请求失败");
      if (data.message?.includes("Token校验失败或已失效")) {
        sendWxPusherMessage({
          msgType: 1,
          app_name: "芒果平台",
          expirePhone: "机器手机号",
  ...
  error => {
    ...
    if (response && response.status) {
      ...
      ElMessage.error("芒果网络连接异常，请稍后再试");
    }
    return Promise.reject(error);
  }
);
