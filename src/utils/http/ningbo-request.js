...
const { ningboToken } = storeToRefs(userInfoAndTokens);
const { removeNingboUserInfo } = userInfoAndTokens;
  ...
  response => {
    ...
    if (
      ...
      if (data.errcode === "205" && data.msg === "登录失效") {
        ...
        ElMessageBox.confirm("宁波影都登录失效，请重新登录", "提示", {
