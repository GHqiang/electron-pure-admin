...
instance.interceptors.request.use(
  config => {
    if (config.url.indexOf("/wanda/") !== -1) {
      ...
      if (IS_DEV) {
        config.url = config.url.replace("wanda", "svpi/wanda-ser");
      } else {
        config.url =
          "http://47.113.191.173:3000" +
          config.url.replace("wanda", "wanda-ser");
      }
    }
...
instance.interceptors.response.use(
  response => {
    ...
    let isErrorByLieRen =
      response.config.url.indexOf("/wanda-ser/") !== -1 && data.code !== 0;
    if (
      isErrorByLieRen &&
  ...
  error => {
    ...
    if (response && response.status) {
      ...
      ElMessage.error("万达网络连接异常，请稍后再试");
    }
    return Promise.reject(error);
  }
);
