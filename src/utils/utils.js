...
const getCinemaFlag = item => {
  ...
  if (
    isMiRuiKuGroup ||
    (["米瑞酷影城", "奋逗影剧院", "米瑞酷激光影城"].some(itemA =>
      cinemNameSpecial(cinema_name).includes(itemA)
    ) &&
      ["北京", "保定"].includes(city_name))
  ) {
    return "miruiku";
  }
  let isSuNingKuGroup = ["苏宁", "c_suning", "苏宁院线"].includes(cinema_group);
  if (
    isSuNingKuGroup ||
    [
      "苏宁影城",
  ...
  let isShouDuKuGroup = ["首都"].includes(cinema_group);
  if (
    isShouDuKuGroup ||
    ["首都电影院", "大观楼影城", "广安门电影院"].some(itemA =>
      cinemNameSpecial(cinema_name).includes(itemA)
    )
  ) {
    return "shoudu";
  ...
  // 蚂蚁和洋葱、哈哈：耀莱成龙。 猎人和芒果：耀莱一线、耀莱二线
    ...
    ["海岸影城"].some(itemA => cinemNameSpecial(cinema_name).includes(itemA)) &&
    ["深圳"].includes(city_name)
  ) {
    return "szhayc";
  } else if (
    ["华夏星光国际影城南山书城店"].some(itemA =>
      cinemNameSpecial(cinema_name).includes(itemA)
    ) &&
    ["深圳"].includes(city_name)
  ) {
    ...
    ["中山IM电影城南朗巨幕店"].some(itemA =>
      cinemNameSpecial(cinema_name).includes(itemA)
    ) &&
    ["中山"].includes(city_name)
  ) {
    return "zsimdyc";
  } else if (
    ["劲松电影院"].some(itemA =>
    ...
    ["中影南方CINITYLED影城宝安华强广场店"].some(itemA =>
      cinemNameSpecial(cinema_name).includes(itemA)
    ) &&
    ["深圳"].includes(city_name)
  ) {
