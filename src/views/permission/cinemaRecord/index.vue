...
const saveCinema = async cinemaInfo => {
  try {
    ...
    if (cinemaInfo.id) {
      ...
      // 此处需增加是否是系统切换判断
      // 1、券类型表自动变更，将app_name相同记录的app_type变更为新的（该表不关联影院id，所以可以自动处理）
      // 2、券库存表无需变更，因为该表没有app_type字段
      // 3、登录信息表自动变更，将app_name相同记录的app_type变更为新的
      // 4、会员卡表手动变更，可删除现有卡列表，重新进行该影院会员卡的同步（因为卡号id、指定影院id可能不同）
      // 5、报价规则表手动变更，可手动进行规则编辑
      // 6、影院映射表手动变更，可先删除该影院的记录，重新进行该影院的映射
