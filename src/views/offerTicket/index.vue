    ...
    async start(fetchDelay, processDelay) {
        ...
        while (this.isRunning) {
            ...
            while (this.queue.length > 0 && this.isRunning) {
                ...
                if (order) {
                    // 处理订单
                    const offerResult = await this.orderHandle(
                        order,
                        processDelay
                    );
                    // 添加订单处理记录
...
const offerRuleMatch = async order => {
    try {
        ...
        let cityRuleList = shadowLineRuleList.filter(item => {
            ...
            if (
                !item.includeCityNames.length &&
                !item.excludeCityNames.length
            ) {
                return true;
            }
        ...
        let cinemaRuleList = cityRuleList.filter(item => {
            if (
                !item.includeCinemaNames.length &&
                !item.excludeCinemaNames.length
            ) {
                return true;
            }
        ...
        let hallRuleList = cinemaRuleList.filter(item => {
            ...
            if (
                !item.includeHallNames.length &&
                !item.excludeHallNames.length
            ) {
                return true;
            }
            if (item.includeHallNames.length) {
                let isMatch = item.includeHallNames.some(hallName => {
                    return (
                        hall_name
                            .toUpperCase()
                            .indexOf(hallName.toUpperCase()) > -1
                    );
                });
        ...
        let filmRuleList = hallRuleList.filter(item => {
            ...
            if (
                !item.includeFilmNames.length &&
                !item.excludeFilmNames.length
            ) {
                return true;
            }
            if (item.includeFilmNames.length) {
                let isMatch = item.includeFilmNames.some(filmName => {
                    return (
                        film_name
                            .toUpperCase()
                            .indexOf(filmName.toUpperCase()) > -1
                    );
                });
...
const getMinAmountOfferRule = async (ruleList, order) => {
    try {
        ...
        if (mixAddAmountRule) {
            ...
            // 会员最终报价
            memberPrice =
                Number(memberPrice) + Number(mixAddAmountRule.addAmount);
