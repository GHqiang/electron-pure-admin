<template>
  <div class="login-jiujin">
    <el-divider content-position="center">华夏久金登录</el-divider>
...
let conPrefix = "【华夏久金登录】——"; // console打印前缀
...
async function getCaptcha() {
  try {
    if (phoneNumber.value.length >= 11) {
      ...
      console.log(conPrefix + "获取图形验证码参数", params);
      const res = await sfcApi.getSmsCode(params);
      console.log(conPrefix + "获取图形验证码返回", res);
      captchaUrl.value = res.codeurl;
      showCaptcha.value = true;
    }
  } catch (error) {
    console.error(conPrefix + "获取图形验证码异常", error);
  }
}
...
async function sendSmsCode() {
  try {
    if (captchaCode.value) {
      ...
      console.log(conPrefix + "获取短信验证码参数", params);
      const res = await sfcApi.getSmsCode(params);
      console.log(conPrefix + "获取短信验证码返回", res);
      showSmsCode.value = true;
      sentSms.value = true;
    }
  } catch (error) {
    console.error(conPrefix + "获取短信验证码异常", error);
  }
}
...
async function autoLogin() {
  try {
    if (smsCode.value && sentSms.value) {
      ...
      console.log(conPrefix + "验证短信并登录参数", params);
      const res = await sfcApi.verifyLogin(params);
      console.log(conPrefix + "验证短信并登录返回", res);
      $emit("loginSuccess", res.data.user_data);
    }
  } catch (error) {
    console.error(conPrefix + "验证短信并登录异常", error);
  }
}
