            ...
            <el-form :model="formData" ref="ruleFormRef" label-width="120px">
                ...
                <el-form-item label="包含影院">
                    <el-select
                        ...
                        :disabled="
                            !!(formData.excludeCinemaNames && formData.excludeCinemaNames.length)
                        "
                        filterable
                        multiple
                        clearable
                ...
                <el-form-item label="排除影院">
                    <el-select
                        ...
                        :disabled="
                            !!(formData.includeCinemaNames && formData.includeCinemaNames.length)
                        "
                        filterable
                        multiple
                        clearable
                ...
                <el-form-item label="包含影厅">
                    ...
                    <span style="color: red">可自定义影厅名称，输入所需名称并点击选择</span>
                </el-form-item>
                ...
                <el-form-item label="包含影片">
                    ...
                    <span style="color: red">可自定义影片名称，输入所需名称并点击选择</span>
                </el-form-item>
                ...
                <el-form-item label="报价类型">
                    <el-radio-group v-model="formData.offerType" @change="offerTypeChange">
                ...
                <el-form-item label="会员日" v-if="formData.offerType === '3'">
                    <el-select
                        ...
                        clearable
                        >
                        <el-option v-for="(item, index) in 31" :key="index" :label="item + '号'" :value="String(item)" />
                    </el-select>
                </el-form-item>
                <el-form-item label="报价金额" v-if="formData.offerType !== '2'">
                    <el-input v-model="formData.offerAmount" placeholder="报价金额" clearable />
                </el-form-item>
                ...
                <el-form-item label="加价金额" v-if="formData.offerType === '2'">
                    <el-input v-model="formData.addAmount" placeholder="加价金额" clearable />
                </el-form-item>
                ...
                <el-form-item label="座位数">
                    <el-select
                        ...
                        clearable
                        >
                        <el-option v-for="(item, index) in 10" :key="index" :label="item" :value="String(item)" />
                    </el-select>
                    <span style="color: red">提示：控制座位数大于X，不进行报价</span>
                </el-form-item>
                <el-form-item label="开场时间限制">
                    <el-input v-model="formData.timeLimit" placeholder="开场时间限制" clearable>
                        <template #append> 单位：小时 </template>
                    </el-input>
                    <span style="color: red">提示：控制距离开场时间小于X小时，不进行报价</span>
                </el-form-item>
...
const excludeCityList = computed(() => {
    ...
    return cityList.value.filter((item) => !formData.includeCityNames.includes(item.name))
})

// 包含/排除影院列表
const cinemaListFilter = computed(() => {
    if (formData.includeCityNames.length) {
        return cinemaList.value.filter((item) => formData.includeCityNames.includes(item.city_name))
